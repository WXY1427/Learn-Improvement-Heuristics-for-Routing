...
class MultiHeadAttention_to_attn(nn.Module):
    ...
    def forward(self, q, exchange, che_mask, h=None, mask=None):
        ...
        Q = torch.matmul(qflat, self.W_query).view(shp_q)
        # Calculate keys and values (n_heads, batch_size, graph_size, key/val_size)
        K = torch.matmul(hflat, self.W_key).view(shp)   ###################################   shp_q
        V = torch.matmul(hflat, self.W_val).view(shp)
...
#         com = compatibility.detach().clone()
#         tt=compatibility[:,:,:gs,:][com[:,:,:gs,:]> -1e10].clone()
#         tt[1-che_mask.byte()] = -np.inf

#         compatibility[:,:,:gs,:][com[:,:,:gs,:]> -1e10]=tt
...
#         best_embedding = self.current_best(now_best[:, None])
    
#         be = best_embedding[:, None, :].expand_as(x)

#         h = x + be
        bs, gs, in_d = x.size()

#         embedding = self.rnn(embedding.view(-1, in_d))
    
#         h = x + embedding.view(bs,gs,in_d)
...
#         fusion = self.one_attn_pre(fusion)
